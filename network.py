...
A module to implent the stochastic gradient descent learning 
algorithm for a feedforward netural network. Gradients are calculated
...
class Network(object):
    ...
    def __init__(self, sizes):
        ...
        and variance 1. Note that the first layer is assumed to be an input
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta,
            test_data=None):
        """Train the netural network using mini-batch stochastic
        gradient descent. The ''training_data'' is a list of tuples
        ''(x, y)'' representing the training inputs and the desired
        outputs. The other non-optional parameters are self-explanatory.
        If ''test_data'' is provide then the epoch, and partial progress
    ...
    def evaluate(self, test_data):
        """Return the number of test inputs for which the netural
        network outputs the correct result. Note that the netural
        network's output is assumed to be the index of whichever
        neutron in the final layer has the highest activation."""
        test_results = [(np.argmax(self.feedforward(x)), y)
                        for (x, y) in test_data]
        return sum(int(x == y) for (x, y) in test_results)

    def cost_derivative(self, output_activations, y):
        """Return the vector of partial derivations \partial C_x /
